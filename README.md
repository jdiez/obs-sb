...
- **Claude Code integration** (optional) with CLAUDE.md context files, automatic inbox processing hooks, auto-journaling on every note change, self-healing review reminders, auto-generated journal reflections, LLM navigation index, wiki lint (orphan/stale detection), raw sources archive, deep ingestion cascades, query-to-note workflow, cognitive governance rules, template protection hooks, 3 skills (`/inbox-process`, `/weekly-review`, `vault-conventions`), and 2 subagents (`vault-linker`, `note-reviewer`)
